...
namespace sgimg {
   ...
   requires (!std::ranges::view<C>)
      ...
      //Nested case
         ...
         auto v = r | std::views::transform([](auto&& elem) {
            return sgimg::to<std::ranges::range_value_t<C>>(elem);
            });
   ...
   constexpr auto to(R&& r, Args&&... args) -> ContainerType {
      return sgimg::to<ContainerType>(std::forward<R>(r), std::forward<Args>(args)...);
   }
   ...
   namespace detail {
      ...
      auto constexpr operator| (R&& r, closure_range<C, Args...>&& c) {
         return std::apply([&r](auto&&... inner_args) {
            return sgimg::to<C>(std::forward<R>(r), std::forward<decltype(inner_args)>(inner_args)...);
            }, std::move(c.args_));
      }
      ...
      auto constexpr operator| (R&& r, closure_ctad<C, Args...>&& c) {
         return std::apply([&r](auto&&... inner_args) {
            return sgimg::to<C>(std::forward<R>(r));
            }, std::move(c.args_));
      }
   }
